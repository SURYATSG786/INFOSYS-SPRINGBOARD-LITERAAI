...
export function register(req, res) {
  try {
    ...
    const preferred_language = body.preferred_language; // learning language (courses/assessment)
    // ui_language: interface/display language. Defaults to the learning language so
    // existing single-language registrations (en-en, hi-hi, ta-ta, te-te, kn-kn) keep
    // working exactly as before without the caller having to send anything new.
    const ui_language = body.ui_language || preferred_language;
    ...
    if (!LANGUAGES.includes(ui_language)) {
      return res.status(400).json({ error: 'Invalid UI language' });
    }
