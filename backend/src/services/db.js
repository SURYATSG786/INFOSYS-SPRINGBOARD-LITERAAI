...
function getDb() {
  if (!dbInstance) {
    ...
    try {
      dbInstance.exec(`ALTER TABLE users ADD COLUMN ui_language TEXT`);
    } catch (_) { /* column already exists */ }
    try {
      // Backfill: existing users get ui_language = preferred_language (unchanged behaviour)
      dbInstance.exec(`UPDATE users SET ui_language = preferred_language WHERE ui_language IS NULL`);
    } catch (_) { /* ignore */ }
...
function rowToUser(row) {
  ...
  return {
    ...
    ui_language: row.ui_language || row.preferred_language,
...
export function createUser(data) {
  ...
  // Learning language (preferred_language) drives courses & assessment content, unchanged.
  // UI language drives interface text only; defaults to learning language when not given
  // so existing single-language flows (ui === learning, e.g. en-en, ta-ta, te-te, kn-kn, hi-hi)
  // behave exactly as before.
  const ui_language = data.ui_language || preferred_language;
  const education_level = data.education_level || 'Primary School';

  const insertUser = db.prepare(`
    INSERT INTO users (
      id, name, email, password, preferred_language, ui_language, education_level,
...
export function updateUser(id, updates) {
  ...
  const ui_language = updates.ui_language !== undefined ? updates.ui_language : existing.ui_language;
