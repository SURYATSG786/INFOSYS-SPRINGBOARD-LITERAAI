...
import { getBilingualAssessment } from '../data/bilingualAssessments.js';
import { getBilingualCourse, loadBilingualCoursesList } from '../data/bilingualCourses.js';

export function loadCourses(lang = null, uiLang = null) {
  if (uiLang && lang && uiLang !== lang) {
    const bilingualList = loadBilingualCoursesList(uiLang, lang);
    if (bilingualList && bilingualList.length > 0) {
...
/** Every assessment/practice item must have full 5-language text + 4 options + answer index. */
...
export function getCoursesByPath(pathKey, lang = null, uiLang = null) {
  return loadCourses(lang, uiLang).filter((c) => c.path === pathKey);
}
...
export function getCourseById(id, lang = null, uiLang = null) {
  if (uiLang && lang && uiLang !== lang) {
    const custom = getBilingualCourse(uiLang, lang, id);
    if (custom) return custom;
  }
  const found = loadCourses(lang, uiLang).find((c) => c.id === id || c.path === id);
  return found || loadCourses('en').find((c) => c.id === id || c.path === id) || DEFAULT_FOUNDATION_COURSE;
}

export function getRecommendedCourses(score, lang = null, uiLang = null) {
  return getCoursesByPath(getPathFromScore(score), lang, uiLang);
}

export function getRecommendedCourse(score, lang = null, uiLang = null) {
  const list = getRecommendedCourses(score, lang, uiLang);
  return list[0] || null;
}

export function getAssessmentQuestions(educationLevel, lang = 'en', uiLang = null) {
  const actualUi = uiLang || lang;
  if (actualUi !== lang) {
    const custom = getBilingualAssessment(actualUi, lang, educationLevel);
    if (custom) return custom;
  }
...
export function scoreAssessment(educationLevel, answers, lang = 'en', uiLang = null) {
  const actualUi = uiLang || lang;
  let questions = null;
  if (actualUi !== lang) {
    questions = getBilingualAssessment(actualUi, lang, educationLevel);
  }
  if (!questions) {
    const bank = loadAssessments().find((a) => a.education_level === educationLevel);
    if (!bank) {
