...
# Dictionary of strict UI Language Questions for all 5 UI languages:
...
for pair_key, pair_courses in courses.items():
    ...
    if ui_lang not in ui_questions_master:
        continue
    ui_map = ui_questions_master[ui_lang]
    for lvl_key, lvl_data in pair_courses.items():
        if lvl_key not in ui_map:
            continue
        q_texts = ui_map[lvl_key]
...
print(f"Successfully updated all questions to 100% UI language across {updated_count} pairs!")
