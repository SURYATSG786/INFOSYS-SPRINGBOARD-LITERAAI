...
import { getVoicePracticeDetails } from '../data/voicePracticeSentences';
...
export default function VoicePracticePage() {
  ...
  const uiLang = user?.ui_language || i18n.language || 'en';
  const learningLang = user?.preferred_language || 'en';
  const isSameLang = uiLang === learningLang;

  const items = getVoicePracticeDetails(learningLang, uiLang);
  const sentences = items.map((i) => i.target);
  ...
  const {
    ...
    stopListening,
    resetTranscript,
  } = useSpeechRecognition(learningLang);

  const currentItem = items[index] || items[0];
  const currentSentence = currentItem.target;
  ...
  useEffect(() => {
    ...
    stopSpeech();
    if (currentSentence) {
      speakText([t('birdGuideVoicePractice'), currentSentence], learningLang, false, true).catch(() => {});
    }
  }, [index, learningLang, resetTranscript, currentSentence, t]);
  ...
  useEffect(() => {
    if (!listening && transcript) {
      ...
      if (res.score >= 70) {
        speakText(t('correctCheer', "Woohoo! That's correct!"), uiLang).catch(() => {});
      } else if (res.score > 0) {
        speakText(t('tryAgainVoice', 'Not quite, try again!'), uiLang).catch(() => {});
      }
    }
  }, [listening, transcript, currentSentence, uiLang, index, t]);

  const handleListenSample = () => {
    stopSpeech();
    speakText(currentSentence, learningLang).catch(() => {});
  };
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="my-2 rounded-xl bg-[#0b6fb8]/8 p-3 text-center border border-[#06304f]/12">
                ...
                {!isSameLang && currentItem.translation ? (
                  <div className="mt-2 text-xs font-extrabold text-purple-900 bg-purple-100/80 rounded-lg py-1 px-2.5 inline-block border border-purple-200">
