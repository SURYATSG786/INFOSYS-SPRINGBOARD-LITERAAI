...
import { SUPPORTED_LANGS, setAppLanguage, setLearningLanguage } from '../i18n';
...
export default function Profile() {
  ...
  const { user, refreshUser, updateLanguages, logout } = useAuth();
  const navigate = useNavigate();
  const [form, setForm] = useState({
    name: user?.name || '',
    preferred_language: (user?.preferred_language || 'en').split('-')[0],
    ui_language: (user?.ui_language || user?.preferred_language || 'en').split('-')[0],
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      refreshUser(next);
      await setAppLanguage(form.ui_language);
      setLearningLanguage(form.preferred_language);
      setMessage(t('save'));
      if (educationChanged) {
        navigate('/assessment');
      }
  ...
  return (
    <div>
      ...
      >
        ...
        <label className="mb-4 block">
          <span className="label">{t('uiLanguage')}</span>
          <select
        ...
        <label className="mb-4 block">
          <span className="label">{t('learningLanguage')}</span>
          <select
            className="input font-extrabold"
