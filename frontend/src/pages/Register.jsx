...
export default function Register() {
  ...
  const [form, setForm] = useState({
    ...
    password: '',
    age: '',
    // learning language: drives courses + initial assessment content
    preferred_language: (i18n.language || 'en').split('-')[0],
    // UI language: drives menus/buttons/messages. Starts equal to the
    // learning language (e.g. en-en, hi-hi, ta-ta, te-te, kn-kn) — the
    // learner can still pick a different UI language independently.
    ui_language: (i18n.language || 'en').split('-')[0],
        ...
        <div className="mx-auto grid max-w-6xl gap-6 md:grid-cols-2 md:items-center md:min-h-[calc(100vh-3rem)]">
          ...
          >
            ...
            <div className="mb-6 grid grid-cols-2 gap-4">
              <label className="block">
                <span className="sky-label">{t('uiLanguage')}</span>
                <select
                  className="sky-select font-extrabold"
                  value={form.ui_language}
                  onChange={(e) => setField('ui_language', e.target.value)}
                >
                  {SUPPORTED_LANGS.map((l) => (
                    <option key={l.code} value={l.code}>{l.label}</option>
                  ))}
                </select>
                <span className="sky-sub mt-1 block text-xs">{t('uiLanguageHint')}</span>
              </label>
              <label className="block">
                ...
                <span className="sky-sub mt-1 block text-xs">{t('learningLanguageHint')}</span>
              </label>
